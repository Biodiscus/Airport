$(function() {
    ...
    // Get all the brands from the database and fill the form with it
    URLUtil.get(BRAND_URL).then(function(arr) {
        var out = "";
        $.each(arr, function(i, item) {
            ...
            out += "value='"+item.id+"'>"+item.name+"</option>";
        });
        form.find("select.brand").html(out);
    });
});